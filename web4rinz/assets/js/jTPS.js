 ...
 */
 (function($) {
        ...
        $.fn.jTPS = function ( opt ) {
                 ...
                 $(this).attr('tableSettings',JSON.stringify(tableSettings))
                 // console.log($(this).attr('tableSettings'))
                // generic pass-through object + other initial variables
                var pT = $(this), page = page || 1, perPages =  tableSettings.perPages, perPage = perPage || perPages[0],
                                ...
                                function (tdInd) {
                                        $(this).addClass('sortableHeader').unbind('click').bind('click',
                                                function () {
                                                        ...
                                                        $(pT).attr('tableSettings').clickCallback();
                                                }
                                        );
                                }
                        );
                ...
                while ( pageSel-- ) 
                        $('>.nav .selectPerPage', this).prepend( ( (pageSel > 0) ? tableSettings.perPageDelim : '' ) + 
                ...
                $('>.nav .selectPerPage', this).prepend( tableSettings.perPageText ).find('.perPageSelector').each(
                        function () {
                ...
                function drawPageSelectors ( target, page ) {
                        ...
                        $('>.nav', target).each(function () {
                                ...
                                $('.pageSelector:lt(' + ( ( page > ( pageCount - 4 ) ) ? ( pageCount - 5 ) : ( page - 2 ) ) + '):not(:first)', this).addClass('hidePageSelector')
                                        .eq(0).after( '<div class="pageSelectorSeperator">' + $(target).attr('tableSettings').perPageSeperator + '</div>' );
                                ...
                                function () {
                                        $(this).bind('click',
                                                function () {
                                                        ...
                                                        var sStep = $(pT).attr('tableSettings').scrollStep * Math.ceil( Math.abs( ( endPos - beginPos ) / perPage ) );
                                                        if ( sStep > perPage ) sStep = perPage;
                                                        var steps = Math.ceil( Math.abs( beginPos - endPos ) / sStep );

                                                        // start scrolling
                                                        while ( steps-- ) {
                                                                $(this).parent().animate({'opacity':1}, $(pT).attr('tableSettings').scrollDelay,
                                                                        function () {
